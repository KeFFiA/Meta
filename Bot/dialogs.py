...
Здесь вы можете:
    ...
    3. Удалить из белого списка""",
        'tokens_others': """<b>Другие токены</b>

Нажав на кнопку с префиксом, например [GetCourse] откроется карточка этого аккаунта.
В этой карточке можно: 

1. Включить\выключить получение отчетов по выбранному токену
2. Удалить токен.
""",

        'tokens': """<b>Список токенов</b>
...
""",
        ...
        'tokens_fb_st2': """Нажав на кнопку с префиксом [FACEBOOK] откроется карточка этого аккаунта,
В этой карточке можно: 

1. Тонко настроить каждый рекламный аккаунт, привязанный к токену.
2. Удалить токен.

Нажав на кнопку, соответсвующую рекламному аккаунту, откроется карточка этого аккаунта.
...
В зависимости от выбора, обновление будет по конкретным аккаунтам.""",
...
Для того, чтобы повторно увидеть актуальный список сервисов воспользуйтесь командой <code>/add_token services</code>""",
        ...
        'last_update': """<b>Последнее обновление</b>
        
Для того, чтобы узнать, когда было последнее обновление сервера, необходимо:

Нажать на кнопку <code>Последнее обновление</code> в главном меню.

Нажав, будет выведен список с последними обновлениями сервера по каждому сервису""",

    },
    'last_update_records': {
        'success': {
            'Facebook' : 'Последнее обновление в <b>{}</b>',
            'eWebinar' : 'Последнее обновление в <b>{}</b>',
            'GetCourse' : 'Последнее обновление в <b>{}</b>'
        },
        'failed': {
            'Facebook' : 'Ошибка! Последняя попытка в <b>{}</b>',
            'eWebinar' : 'Ошибка! Последняя попытка в <b>{}</b>',
            'GetCourse' : 'Ошибка! Последняя попытка в <b>{}</b>'
        }
    }
}
